...
const NOTIFY_GROUPS: { key: NotifyGroup; label: string; desc: string }[] = [
  ...
  { key: 'dadiyal',  label: 'Dadiyal',   desc: "Father's side" },
  { key: 'naniyal',  label: 'Naniyal',   desc: "Mother's side" },
  { key: 'internal', label: 'Internal',  desc: 'Blood relatives' },
  { key: 'extended', label: 'Extended',  desc: 'Full family' },
]

const EVENT_TYPES = [
  'BIRTHDAY', 'ANNIVERSARY', 'WEDDING', 'TRIP',
  'GATHERING', 'ACHIEVEMENT', 'OTHER',
] as const
...
const TYPE_LABELS: Record<EventType, string> = {
  ...
  TRIP: 'Trip', GATHERING: 'Gathering', ACHIEVEMENT: 'Achievement', OTHER: 'Other',
}
...
export default function CreateEventScreen({ treeId, persons, onSave, onCancel, initialEvent }: Props) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      // Upload photos if any (non-blocking — navigate immediately, uploads continue)
      if (photos.length > 0) {
        uploadPhotos(treeId, saved.id, photos).catch(() => {})
      }
      onSave(saved)
    } catch (e) {
      setError(e instanceof ApiError ? e.message : `Failed to ${isEdit ? 'update' : 'create'} event.`)
    } finally {
      setLoading(false)
    }
  }
    ...
    <View style={styles.root}>
      ...
      <ScrollView style={styles.scroll} keyboardShouldPersistTaps="handled">
                ...
                return (
                  <TouchableOpacity
                    ...
                    style={[styles.typeChip, {
                      // @ts-ignore
                      background: selected ? `linear-gradient(135deg, ${c0}, ${c1})` : C.surfaceEl,
                    }]}
                  >
                    <Text style={[styles.typeChipText, { color: selected ? '#fff' : C.textPrimary }]}>
                      {TYPE_LABELS[t]}
                    </Text>
                  </TouchableOpacity>
                )
        ...
        <Text style={styles.photoHint}>
          {photos.length === 0
            ? 'Add photos from your device · auto-optimised before upload'
            : `${photos.filter(p => p.blob).length}/${photos.length} ready${anyCompressing ? ' · optimising…' : ''}`
          }
        </Text>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.photoRow}>
          {/* + Add tile */}
          <TouchableOpacity style={styles.photoAddTile} onPress={handleAddPhotos} activeOpacity={0.7}>
        ...
        {!isEdit && (
          <>
            ...
            <Text style={styles.dateHint}>Can be a past or future date — we'll show both the event date and when you posted it.</Text>
            ...
            <input
              ...
              style={{
                fontSize: 15, color: C.textPrimary, backgroundColor: C.surfaceEl,
          ...
          <>
            ...
            <View style={styles.notifyGrid}>
              {NOTIFY_GROUPS.map(g => (
                ...
                >
                  <Text style={[styles.notifyChipLabel, notifyGroup === g.key && styles.notifyChipLabelActive]}>
                    {g.label}
                  </Text>
                  <Text style={[styles.notifyChipDesc, notifyGroup === g.key && styles.notifyChipDescActive]}>
                    {g.desc}
                  </Text>
                </TouchableOpacity>
              ))}
...
const styles = StyleSheet.create({
  ...
  saveBtn: {
    backgroundColor: C.accent, paddingHorizontal: 16, paddingVertical: 8,
  ...
  error: {
    ...
    padding: 12, margin: 16, borderRadius: 8, fontSize: 13, textAlign: 'center',
  },
  ...
  section: {
    ...
    letterSpacing: 1, paddingHorizontal: 16, marginTop: 20, marginBottom: 8,
  },

  typeRow: { paddingHorizontal: 12, marginBottom: 4 },
  typeChip: {
    paddingHorizontal: 14, paddingVertical: 8, borderRadius: 20,
    marginHorizontal: 4, minWidth: 80, alignItems: 'center',
  },
  typeChipText: { fontSize: 13, fontWeight: '600', textAlign: 'center' },

  banner: {
    marginHorizontal: 16, marginVertical: 12, height: 120,
    borderRadius: 12, justifyContent: 'flex-end', padding: 16,
  },
  ...
  label: {
    ...
    marginBottom: 4, marginTop: 12, paddingHorizontal: 16,
  },
  dateHint: {
  ...
  tagChip: {
    paddingHorizontal: 14, paddingVertical: 8, borderRadius: 20,
    borderWidth: 1.5, borderColor: C.border, backgroundColor: C.surfaceEl,
  },
  ...
  notifyChip: {
    ...
    backgroundColor: C.surfaceEl, minWidth: '30%', flexGrow: 1,
  },
