...
export const eventRepository = {
  ...
  // Event.date is always stored as a UTC instant (mobile sends `${date}T00:00:00.000Z`),
  // so "today" must be computed in UTC too — mirrors memory-recall.job.ts's query,
  // scoped to a single tree instead of scanning every tree in the system.
  async findOnThisDay(treeId: string, month: number, day: number, year: number) {
