...
export function buildAuthService(fastify: FastifyInstance) {
  async function issueTokens(userId: string, uniqueUserId: string) {
    ...
    const refreshPayload = { sub: userId, uniqueUserId, tokenId, type: 'refresh' } as unknown as { sub: string; uniqueUserId: string }
  ...
  return {
    async signup(input: SignupInput) {
      ...
      if (usernameTaken) throw Object.assign(new Error('Username already taken'), { statusCode: 409 })
    ...
    async refresh(rawToken: string) {
      let payload: { sub: string; uniqueUserId: string; tokenId: string; type: string }
      try {
        payload = fastify.jwt.verify(rawToken) as typeof payload
      } catch {
      ...
      if (!exists) throw Object.assign(new Error('Refresh token revoked'), { statusCode: 401 })

      // Rotate: delete old, issue new
      await redis.del(key)
      return issueTokens(payload.sub, payload.uniqueUserId)
    },

    async logout(rawToken: string) {
      try {
        const payload = fastify.jwt.verify(rawToken) as { sub: string; tokenId?: string; type?: string }
        if (payload.tokenId) {
