...
const PERSON_SELECT = {
  ...
  dateOfBirth: true, isDeceased: true, profilePicUrl: true, createdAt: true,
  linkedUserId: true,
...
export const personRepository = {
  ...
  update(personId: string, data: Partial<{
    firstName:     string
    lastName:      string
    gender:        'MALE' | 'FEMALE' | 'OTHER'
    dateOfBirth:   Date | null
  ...
  // Edges
  createEdge(treeId: string, fromPersonId: string, toPersonId: string, relationType: 'PARENT' | 'SPOUSE' | 'SIBLING') {
    return db.relationshipEdge.create({
      data: { treeId, fromPersonId, toPersonId, relationType },
    })
  },

  deleteEdge(treeId: string, fromPersonId: string, toPersonId: string, relationType: 'PARENT' | 'SPOUSE' | 'SIBLING') {
